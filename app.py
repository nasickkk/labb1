...
# Тестовые ключи Google reCAPTCHA v2.
# Для локальной проверки лабораторной их достаточно.
# Для реального сайта на PythonAnywhere лучше заменить на свои ключи.
...
def denoise_image(image, method, smoothing_parameter):
    ...
    Вариант 1 разрешает выбрать метод фильтрации самостоятельно.
    Пользователь задает параметр сглаживания, который влияет на силу фильтра.
    """
...
def index():
    if request.method == "POST":
        ...
        save_noise_distribution(original_image, filtered_image, noise_map_path, noise_hist_path)
